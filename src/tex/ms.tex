...
We implement several constraints on the space of expressions, to rule out uninterpretable combinations of operators.
We impose the following nested constraints:
\begin{enumerate}
    \item No operator with arity of 1 (i.e., taking a single argument) may be nested with another arity-1 operator, unless the operator is square.
    \begin{enumerate}
        \item However, a square can only be nested inside itself once.
    \end{enumerate}
    \item A division operator may only be nested inside itself once.
    \item A power operator may not contain any arity-1 operator, except a square.
\end{enumerate}
