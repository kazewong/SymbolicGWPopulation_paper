...
In this work, we demonstrate the use of symbolic regression to distill such flexible models into interpretable analytic expressions.
...
\includegraphics[width=\textwidth]{figures/Page2.pdf}
...
Today there exist 79 official events, the majority of them being Binary Black Holes (BBHs).
As more GW events are observed, more features in the distribution of GW events become resolved and available for discovery.
...
By comparing the model to the data, one can measure the parameters which characterize the model, such as the spectral index. \todo{Could make a statement about how power laws typically have some astrophysical meaning - hence a good operator, and why they are used in population models.}

The central issue with this paradigm is the lack of scalability to a more complex dataset, since construction of sensible phenomenological models requires manual intervention.
\todo{Weird transition here.}
...
the community is looking to develop an interpretable model that is flexible enough to explain the increasing rich set of features in the catalog. 


Besides such physically motivated models, there have been studies using much more flexible models such as a Gaussian Mixture Model (GMM) to analyze the catalog \cite{Tiwari:2020vym}.
...
For example, consider if one wished to assess which parameter in a fitted GMM is the most important: one may apply a Principal Component Analysis (PCA) to the set of parameters and find the most important component.
However, the PCA basis for this dataset is not a basis that is directly related to any astrophysical process,
so having the relative importance in that form does not provide insight into astrophysics. \todo{This makes no sense. Wouldn’t you look at the amplitude?}

Symbolic regression (SR) is a machine learning technique which searches to space of analytic expressions to fit a dataset---this algorithm is both flexible and interpretable. SR can fit  many different patterns, while at the same time presenting the pattern in a simple concise way in the language of existing models—i.e., analytic expressions. SR complements the weakness of a flexible model in understanding the distribution of GW.
...
Specifically, we run SR to approximate the posterior predictive distribution produced by the Gaussian mixture model.
...
Whereas one can fit most ML algorithms to data by optimizing a fixed set number of real parameters, for symbolic regression, one typically uses using a ``genetic algorithm.'' This genetic algorithm arranges symbols in a tree, evaluates the resultant expression, and repeats this process until a particular goal is met.
Since models for physical phenomena are often specified in a language of concise mathematical expressions, symbolic regression can produce very interpretable models compared to traditional ML, as the ``trained model'' generated by symbolic regression is in the same \textit{language} as existing physical models (i.e., analytic expressions).
Therefore, symbolic regression can produce insights for science in a way that traditional ML cannot.

% Give history
%Symbolic regression has a disconnected history,
% with some early work going back to \mc{langley citations}.
%The first application of genetic algorithms
%to symbolic regression was done by \mc{koza}, before this class of algorithms was popularized in the sciences by \mc{lipson}.
% \mc{Cite examples in astronomy.}
% \mc{Describe how PySR works.}
...
and $\ell(\hat{y}_i, y_i) = \qty(\hat{y}_i - y_i)^2$. \todo{What complexities were chosen?}
...
Understanding the population of GW events supports out understanding of the distribution of source properties, such as the mass distribution of black holes \cite{2019PASA...36...10T,Vitale:2020aaz}.
...
In the middle panel \todo{Shouldn’t it be left-middle?}, we show a GMM fitted to the data given by \cite{Tiwari:2020vym}.
...
We choose three accuracy levels to select three different equations in our analysis \todo{Specify how the thresholds were chosen}.
The equations selected under these criteria are tabulated in table \ref{tab:equations}.
Note that we take the absolute value of the equations in order to make figure \ref{fig:Gaussian_m1_fit}.\todo{Is this still true?}
...
Our results suggest any additional features such as tapering of the continuum or wiggles between the two peaks are not statistically significant \todo{I would lighten the language of this part - SR isn’t exactly statistically rigorous}.
...
Our method can be a natural way to explore and to explain these correlations. \todo{Say something about how this can be used for other astrophysical phenomena!}
