...
ax.plot(X['M'].values[:-2],np.exp(X['logP'])[:-2],label='Gaussian Mixture Data',lw=5)
ax.fill_between(X['M'].values[:-2],np.exp(X['logP_low'][:-2]),np.exp(X['logP_high'][:-2]),alpha=0.14,color='C0')
ax.plot(X['M'].values[:-2],np.abs(y1[:-2]),label='PySR,'+label1,lw=3)
ax.plot(X['M'].values[:-2],np.abs(y2[:-2]),label='PySR,'+label2,lw=3)
ax.plot(X['M'].values[:-2],np.abs(y3[:-2]),label='PySR,'+label3,lw=3)
#plt.plot(X['M'].values[:-2],y[:-2]-np.exp(X['logP'])[:-2])
ax.set_yscale('log')
...
plt.savefig("./GWTC3_Gaussian_m1_fit.pdf",bbox_inches='tight')